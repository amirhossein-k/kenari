...
const ClientProductPage = ({ product, errorMessage }: ProductProps) => {
  const [selectedKeys, setSelectedKeys] = useState<Selection>(new Set<Key>(['text']));
...
setPartTwo(!partTwo)
 ...
 const handleBack = () => {
  ...
  return (
    <div className="w-full lg:w-[50%] mx-auto p-4 flex flex-col gap-8 bg-red-300 relative h-screen" dir="rtl">
      {errorMessage && (
...
<div className="">part</div>
  ...
  >
    {itemList.map((item) => (
      <ListboxItem key={item.title}  className={`w-full  text-xl ${selectedValue===item.title ?"bg-red-400":""}`}>
        <div className={`text-lg px-3 py-4 h-full ${selectedValue===item.title ?"bg-red-400":""}`}>
      ...
      <div className="navbot absolute bottom-12 w-full gap-3 right-0 flex bg-gray-300 p-3">
        <button className='w-[54%] h-full bg-green-300 py-4 rounded-lg'
